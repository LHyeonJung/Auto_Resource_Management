...
const { loadSpreadsheet, localesPath, getPureKey, ns, creds, spreadsheetDocId, publicSheetId, NOT_AVAILABLE_CELL } = require("../index");

/* ==== READ ME ====
    [ 업로드 파일 위치 ]
      (DCC Web Resource Management 시트)
      ../index.js 파일내의 DCC Web Resource Management 문서 관련> spreadsheetDocId, publicSheetId 참조
...
async function consistencyCheck(doc, keyMap) {
  ...
  let sheet = doc.sheetsById[publicSheetId];
  ...
  //[시트 다시 로드]
  sheet = doc.sheetsById[publicSheetId];
  ...
  console.log("==1. PUBLIC 키 정합성 체크 완료==");
}

// [대상 시트가 없는 경우, 새로 생성]
async function addNewSheet(doc, title, publicSheetId) {
  const headerValues = ["Key", "KO_KR", "EN_US", "KR_Status"];
...
async function updateTranslationsFromKeyMapToSheet(doc, keyMap) {
  try {
    ...
    if (!sheet) {
      sheet = await addNewSheet(doc, title, publicSheetId);
    }
...
function gatherKeyMap(keyMap, lng, json) {
  ...
  for (const [keyWithPostfix, translated] of Object.entries(targetData)) {
    ...
    if (!key.includes("err_") && !key.includes("log_")) {
      // PUBLIC 타입의 리소스만 추출
      if (!keyMap[key]) {
        keyMap[key] = {};
      }
...
async function updateSheetFromJson(lngs) {
  const doc = await loadSpreadsheet("PUBLIC", true);

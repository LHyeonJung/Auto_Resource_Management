...
const { loadSpreadsheet, localesPath, getPureKey, ns, creds, spreadsheetDocId, logSheetId, NOT_AVAILABLE_CELL } = require("../index");

/* ==== READ ME ====
    [ 업로드 파일 위치 ]
      (DCC Web Resource Management 시트)
      ../index.js 파일내의 DCC Web Resource Management 문서 관련> spreadsheetDocId, logSheetId 참조

    [ 업로드 파일 실행 방법 ]
      package.json > script에 정의
          - 형태: "node {업로드파일 경로} {업로드 대상 언어} {대상 리소스 컬럼 번호} {대상 Status 컬럼 번호}"
          - 예시: "dccLogResourceUpload:i18n": "node translationTool/upload/LogUpload.js KO_KR D G",
...
async function consistencyCheck(doc, keyMap) {
  ...
  let sheet = doc.sheetsById[logSheetId];
  ...
  //[시트 다시 로드]
  sheet = doc.sheetsById[logSheetId];
  ...
  console.log("==1. LOG 키 정합성 체크 완료==");
}

// [대상 시트가 없는 경우, 새로 생성]
async function addNewSheet(doc, title, logSheetId) {
...
async function updateTranslationsFromKeyMapToSheet(doc, keyMap) {
  try {
    ...
    if (!sheet) {
      sheet = await addNewSheet(doc, title, logSheetId);
    }
...
async function updateSheetFromJson(lngs) {
  const doc = await loadSpreadsheet("LOG", true);

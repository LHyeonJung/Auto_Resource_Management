const fs = require("fs");
const { loadSpreadsheet, getPureKey, localesPath, ns, errorSheetId, NOT_AVAILABLE_CELL } = require("../index");

/*  ==== READ ME ====
    [ DCC Web Resource Management 시트 다운로드 파일 실행 방법 ]
      package.json > script에 정의
          - 형태: "node {파일 경로} {다운로드 대상 언어}"
          - 예시: "node translationTool/download/ErrorDownload.js KO_KR"
...
async function fetchTranslationsFromSheetToJson(doc) {
  const sheet = doc.sheetsById[errorSheetId];
  if (!sheet) {
    return {};
  }
...
function statusCheck(lngsMap, localeJsonFilePath, lng) {
  ...
  // public, log 리소스의 경우 그대로 유지
  const public_log_Resorce = sourceResourceArr.filter((x) => !x[0].includes("err_"));
  public_log_Resorce.forEach((element) => {
    const key = element[0];
...
async function updateJsonFromSheet() {
  const doc = await loadSpreadsheet("ERROR", false);
